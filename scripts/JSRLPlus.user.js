...
// - Code cleanup
// 0.6.0
// - Made the TV window movable
// - Made the Song List window movable
// - Started implementing a Windows 95 style
// - Shifted some sidebar buttons to the Taskbar
...
function createTaskbarPrograms()
{
    // add new #programs div to the #information div
    let programsDiv = document.createElement("div");
    programsDiv.id = "programs";
    
    // add the new div to the #information div after #programInformationText
    let informationDiv = document.getElementById("information");
    let programInformationText = document.getElementById("programInformationText");
    informationDiv.insertBefore(programsDiv, programInformationText.nextSibling);

    // move the #chatButton to the #programs div, inside a #chatProgram div
    let chatProgram = document.createElement("div");
    chatProgram.id = "chatProgram";
    chatProgram.className = "program";
    chatProgram.appendChild(document.getElementById("chatButton"));
    document.getElementById("programs").appendChild(chatProgram);

    // repeat with #tvButton
    let tvProgram = document.createElement("div");
    tvProgram.id = "tvProgram";
    tvProgram.className = "program";
    tvProgram.appendChild(document.getElementById("tvButton"));
    document.getElementById("programs").appendChild(tvProgram);
}
...
setInterval(function () {
    ...
    try {
        ...
        if ((!songListHover || !songListMoveable) && !titleOverlayVisible) {
            drawSongList();

            if (!songListMoveable && windowsMode) {
                moveableSongListWindow();
                songListMoveable = true;
            }
        }
    } catch (e) { }
}, 2000);
...
function drawSongList() {
    ...
    if (!refresh) {
        ...
        songListTitle.id = "songListTitle";
        songListTitle.classList.add("songListTitle");

        // create text for songlist title
        let songListTitleText = document.createElement("div");
        songListTitleText.id = "songListTitleText";
        songListTitleText.classList.add("songListTitleText");
        songListTitle.appendChild(songListTitleText);

        songList.appendChild(songListTitle);

        if (windowsMode) {
            songList.classList.add("windows");

            // add a #songListCloseButton to the title
            let songListCloseButton = document.createElement("div");
            songListCloseButton.id = "songListCloseButton";
            songListCloseButton.classList.add("closeButton");
            songListTitle.appendChild(songListCloseButton);
        }
    }
    else {
    ...
    // replace songlist title text
    let songListTitleText = songListTitle.getElementsByClassName("songListTitleText")[0];
    songListTitleText.innerHTML = `Now Playing on ${getStationDisplayName(currentStation)}`;
...
function moveableChatWindow() {
    ...
    if (windowsMode)
        chatWindow.classList.add("windows");
...
function moveableSongListWindow() {
    console.log("Making song list window moveable");

    // gibe the .songListTitle an id so we can move it
    let songList = document.getElementById("songList");
    let songListTitle = songList.getElementsByClassName("songListTitle")[0];
    songListTitle.id = "songListTitle";

    // make song list window moveable
    dragElement(songList, "Title");
}

function moveableTVWindow() {
    console.log("Making TV window moveable");

    // create a div called tvWindow
    let tvWindow = document.createElement("div");
    tvWindow.id = "tvWindow";
    tvWindow.classList.add("windows");

    // create a title for the tvWindow
    let tvTitle = document.createElement("div");
    tvTitle.id = "tvWindowTitle";
    // create a text node for the title, and a close button
    let tvTitleText = document.createTextNode("TV");
    let tvTitleCloseButton = document.createElement("button");
    tvTitleCloseButton.id = "tvTitleCloseButton";
    tvTitleCloseButton.innerHTML = "X";
    // add the text node and close button to the title
    tvTitle.appendChild(tvTitleText);
    tvTitle.appendChild(tvTitleCloseButton);

    // add the title to the tvWindow
    tvWindow.appendChild(tvTitle);
 
    // move tvFrame into another div called tvWindow
    let tvFrame = document.getElementById("tvFrame");
    tvFrame.parentNode.insertBefore(tvWindow, tvFrame);
    tvWindow.appendChild(tvFrame);

    // make tvWindow moveable
    dragElement(tvWindow, "Title");
}
...
function dragElement(elmnt, grabName = null) {
    ...
    function dragMouseDown(e) {
        ...
        // find all elements with the class "windows" and set this element to the highest z-index, but don't set any to 0
